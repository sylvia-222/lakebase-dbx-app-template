...
try:
    ...
    error_message = str(e)
    if "Cluster id is required but was not specified" in error_message:
        print(f"❌ Failed! Databricks Connect couldn't automatically find a compute resource: {e}")
        print("Suggestion: Ensure 'serverless_compute_id = auto' is correctly added")
        print("            under the [DEFAULT] profile in your '~/.databrickscfg' file.")
        print("            This setting allows Connect to use available Serverless compute for interactive sessions.")
        print("            Also ensure your workspace/user has access to Serverless compute.")
    else:
